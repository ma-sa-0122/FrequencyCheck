...
namespace Karaoke
{
    public partial class Karaoke : Form
    {
        ...
        {
            ringBuffer = new double[fftSize]; // 過去データ保持用
            ringPos = 0;
        ...
        private int ringPos;
        private int hopSize = 256;  // 1回のシフト量（オーバーラップ率 75%）
        ...
        private void WaveIn_DataAvailable(object sender, WaveInEventArgs e)
        {
            ...
            {
                double val = buffer[i] / (double)short.MaxValue;
                processed[i] = val;

                lock (ringLock)
                {
                    ringBuffer[ringPos] = val;
                    ringPos = (ringPos + 1) % ringBuffer.Length;
                }
            }
            ...
            {
                // 古いデータを捨てる
                while (analysisQueue.TryDequeue(out _)) { }
                analysisQueue.Enqueue(processed);
            }
        }
