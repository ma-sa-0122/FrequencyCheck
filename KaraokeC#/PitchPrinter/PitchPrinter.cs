...
namespace Karaoke
{
    public partial class PichPrinter : Form
    {
        ...
        {
            SetupChart(chartPitch, 0, 12, ARRAY_RANGE, ARRAY_RANGE, "Pitch");
            ...
            for (int i = 0; i < 12; i++)
            {
                CustomLabel label = new CustomLabel(i - 0.5, i + 0.5, noteNames[i], 0, LabelMarkStyle.None);
        ...
        private void WaveIn_DataAvailable(object sender, WaveInEventArgs e)
        {
            ...
            // ピッチとcumAve検出
            (double pitch, double[] auxData) = DetectPitchAndCumAve(processed);

            // pitchArray更新
            pitchArray[pitchIndex % ARRAY_RANGE] = pitch;
            pitchIndex++;

            // ピッチチャート描画
            Invoke((Action)(() =>
            {
                var series = chartPitch.Series[0];
                series.Points.Clear();
