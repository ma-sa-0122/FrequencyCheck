...
void gnuplotSet() {
    ...
    fprintf(gp2, "set cbrange [0:500000]\n");
    fprintf(gp2, "set xrange [-0.5:0.5]\n");
    fprintf(gp2, "set yrange [32.7:%d]\n", SAMPLING_RATE/2);
    fprintf(gp2, "set y2range [32.7:%d]\n", SAMPLING_RATE/2);
...
void CALLBACK waveInProc(HWAVEIN hwi, UINT uMsg, DWORD_PTR dwInstance, DWORD_PTR dwParam1, DWORD_PTR dwParam2) {
    switch (uMsg) {
        ...
        case WIM_DATA:   // バッファが満杯になったとき
            ...
            // 周波数情報をカラーグラフとして表示
            fprintf(gp2, "plot '-' matrix with image\n");
            for (int i = 32; i < SAMPLING_RATE/2; i++) fprintf(gp2, "%lf 0\n", F[i].real*F[i].real + F[i].imag*F[i].imag);  fprintf(gp2, "e\n");
            fflush(gp2);
            cnt++;
            break;
